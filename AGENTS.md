Supported deploy targets are Android 8+ and iOS. 

Development target you should be using when doing builds is `macos`

Do **not** execute `flutter run`. Instead let user run and test the app and present a query to user to provide feedback.

Before proposing to accept_solution, please run: `sh ./contrib/pre-commit`. Fix any issues remaining.
...
* **State Management:**
  ...
  * **Project convention:** Use `get_it`, `watch_it`. Use skills for that
  * **Native:** Use `ValueNotifier`, `ChangeNotifier` else way.
